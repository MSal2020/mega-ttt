  ...
  for (const [dr,dc] of dirs) {
    ...
    for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
      ...
      for (let i = 0; ; i++) {
        ...
        if (cell && match(cell.owner) && !cell.wall && cell.visible !== false && !cell.scored) cells.push([nr,nc]);
        else break;
      }
      if (cells.length >= len) {
        found.push(cells);
...
export function scoreAndMark(board, pc, lineLen, prevScores, teams = null) {
  ...
  while (found) {
    ...
    for (const g of groups) {
      ...
      if (lines.length > 0) {
        // Credit every member of the group so all teammates share the same score.
