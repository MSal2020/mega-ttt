...
// powerId: "takeover" | "block" | "ghost"
export function aiPickPowerAction(board, aiPlayer, powerId, lineLen, playerCount) {
  const n = board.length;
  if (powerId === "takeover") {
    // Find opponent tile that contributes most to their threats
    let best = -Infinity, bestCell = null;
    for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
      const cell = board[r][c];
      if (!cell || cell.wall || cell.owner === aiPlayer || cell.visible === false || cell.scored) continue;
      // Temporarily remove, see how much opponent loses
      const temp = { ...cell };
      board[r][c] = null;
  ...
  if (powerId === "block") {
    // Find empty cell that best blocks strongest opponent threat
    let best = -Infinity, bestCell = null;
    for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
      ...
      const score = oppMax + Math.random() * 5;
      if (score > best) { best = score; bestCell = [r, c]; }
    }
    return bestCell;
  }
  if (powerId === "ghost") {
    // Place ghost somewhere it extends our own line
    let best = -Infinity, bestCell = null;
    for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
      if (board[r][c]) continue;
      const myScore = cellThreatScore(board, r, c, aiPlayer, lineLen, true);
      const score = myScore + Math.random() * 5;
      if (score > best) { best = score; bestCell = [r, c]; }
    }
    return bestCell;
  }
