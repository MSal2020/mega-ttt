...
// Monochrome glass system — players differentiated by shape + fill-style, not color.
// `fill`/`light`/`ring` retained for legacy callers (tiny colour dots, banners);
// these map onto warm grayscale tones of the same ink family.
export const PLAYERS = [
  { fill: "#1A1714", light: "#EAE4D8", name: "Circle",   ring: "#4A453E", glyph: "circle",   fillStyle: "solid"    },
  { fill: "#3A342C", light: "#E6DFD2", name: "Triangle", ring: "#6A6156", glyph: "triangle", fillStyle: "ring"     },
  { fill: "#5F564A", light: "#DFD7C8", name: "Square",   ring: "#857A6A", glyph: "square",   fillStyle: "halftone" },
  { fill: "#85796A", light: "#D6CDBD", name: "Diamond",  ring: "#A89C89", glyph: "diamond",  fillStyle: "dotted"   },
];

// Fixed team partition for 4-player team mode: slots 0 & 2 vs 1 & 3.
export const TEAM_SLOTS = [[0, 2], [1, 3]];
// Accent hues so teams are visible at a glance against the monochrome shapes.
export const TEAM_ACCENTS = ["#C8544A", "#4A7BF7"];
export const TEAM_NAMES = ["Team A", "Team B"];
export function teamOf(slot) { return slot % 2 === 0 ? 0 : 1; }
...
export function aiPlanPowerAction(board, aiPlayer, powerId, lineLen, playerCount, blocks = [], globalTurn = 1) {
  const n = board.length;
  if (powerId === "takeover") {
    ...
    for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
      ...
      // Defensive value: how much threat is removed by taking this tile away.
      board[r][c] = null;
      const removedOppThreat = cellThreatScore(board, r, c, original.owner, lineLen, true);

      // Offensive value: how much this converted tile helps the AI.
      board[r][c] = { owner: aiPlayer, visible: true };
      const ownThreat = cellThreatScore(board, r, c, aiPlayer, lineLen, true);
      const createsImmediateWin = wouldCompleteLineAt(board, r, c, aiPlayer, lineLen);

      board[r][c] = original;

      const score = removedOppThreat * 1.15 + ownThreat * 1.25 + (createsImmediateWin ? 25000 : 0);
      if (score > best) { best = score; bestCell = [r, c]; }
    }
    return bestCell ? { action: bestCell, score: best } : null;
  }
  if (powerId === "block") {
    ...
    for (let r = 0; r <= n - size; r++) for (let c = 0; c <= n - size; c++) {
      ...
      for (let dr = 0; dr < size; dr++) for (let dc = 0; dc < size; dc++) {
        ...
        if (!board[rr][cc]) {
          ...
          for (let p = 0; p < playerCount; p++) {
            ...
            if (wouldCompleteLineAt(board, rr, cc, p, lineLen)) oppMax = Math.max(oppMax, 3000);
          }
          areaScore += oppMax;
        }
      }
      ...
      areaScore += emptyInArea * 2;
      if (areaScore > best) { best = areaScore; bestAnchor = [r, c]; }
    }
    return bestAnchor ? { action: bestAnchor, score: best } : null;
  }
  if (powerId === "teleport") {
    ...
    for (const [sr, sc] of sources) {
      ...
      for (const [dr, dc] of dests) {
        ...
        const createsImmediateWin = wouldCompleteLineAt(board, dr, dc, aiPlayer, lineLen);
        const score = destVal - srcVal * 0.6 + (createsImmediateWin ? 18000 : 0);
        if (score > best) { best = score; bestMove = { from: [sr, sc], to: [dr, dc] }; }
      }
      board[sr][sc] = temp;
    }
    return bestMove ? { action: bestMove, score: best } : null;
  }
  return null;
}

export function aiPickPowerAction(board, aiPlayer, powerId, lineLen, playerCount, blocks = [], globalTurn = 1) {
  const plan = aiPlanPowerAction(board, aiPlayer, powerId, lineLen, playerCount, blocks, globalTurn);
